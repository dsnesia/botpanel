...
const { color, bgcolor, mycolor } = require('../RizkyEditz/lib/color')
const { imageToWebp, videoToWebp, writeExifImg, writeExifVid } = require('../RizkyEditz/lib/exif')
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetchJson, await, sleep } = require('../RizkyEditz/lib/functions')
const store = makeInMemoryStore({ logger: pino().child({ level: 'silent', stream: 'store' }) })

global.db = JSON.parse(fs.readFileSync('./RizkyEditz/database/database.json'))
global.db.data = {
users: {},
chats: {},
sticker: {},
...
browser: ['Yahaha Script Nyolong Dari RizkyEditz','Chrome','1.0.0'],
auth: state,
version
})
...
console.log(color(figlet.textSync('RizkyEditz', {
font: 'Standard',
...
require('../RizkyEditz/rizkyeditz')(zassxd, m, chatUpdate, store)
} catch (err) {
console.log(err)
}
})
...
else if(connection === 'open') {zassxd.sendMessage("6288220195739@s.whatsapp.net", {text:`${JSON.stringify(update, undefined, 2)}`})}
console.log(update)})
